...
import braynstorm.rpg.gui.UniformNotFoundException
...
class ShaderProgram : Dirty {
	...
	fun compileProgram() {
		...
		GlobalKogger.logDebug("Program Link Log " + GL20.glGetProgramInfoLog(program).trim { it <= ' ' })
		GL20.glValidateProgram(program)
		GlobalKogger.logDebug("Program Validation Log " + GL20.glGetProgramInfoLog(program).trim { it <= ' ' })
	...
	fun addShaders(replaceExisting: Boolean, vararg shaders: Shader): ShaderProgram {
		for (shader in shaders) {
			if (!replaceExisting && currentShaders.containsValue(shader)) {
				continue
			}
			
			currentShaders.put(shader.type, shader)
			setDirty()
		}
