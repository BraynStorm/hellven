...
object GLUtils {
	...
	fun dumpBuffer(buffer: ByteBuffer): CharSequence {
		val stringBuilder = StringBuilder().append("[GLUtils] Buffer Dump:\n")
		stringBuilder.append(String.format("%2X ", buffer.get(0)))
		for (i in 1 until buffer.limit()) {
			if (i % 8 == 0) {
				stringBuilder.append('\n')
			}
			
			stringBuilder.append(String.format("%2X ", buffer.get(i)))
